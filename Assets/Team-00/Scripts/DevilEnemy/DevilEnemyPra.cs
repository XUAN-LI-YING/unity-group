...
public class DevilEnemyPra : MonoBehaviour
{
  public static DevilEnemyPra instance;
  public GameObject catPrefab;       //第一關
   public GameObject DevilPrefab;        //第二關
 ...
 void create()           //第二關       
  {
    ...
    {
      if(createNumber<16) //大隻總共只能複製出5隻
      {
         GameObject go = Instantiate(catPrefab) as GameObject;
        //定義產生位置
        go.transform.position = new Vector3(-104, 23.4f, 6);
        this.delta = 0;
        createNumber+=1;

      }
      else if(createNumber>=16 && createNumber<17)
      {
  ...
  public void addEnemyDie()
  {
    ...
    if(enemyDie>=16)                                    //第二關
    {
      SceneManager.LoadScene("gameWin3"); 
    }
